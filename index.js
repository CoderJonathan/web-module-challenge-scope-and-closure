...
/*Task 1: counterMaker()
  ...
  1. What is the difference between counter1 and counter2?
    ...
    In counter1 the variables are done within the functions scope and counter2 the variables are globally established.

  2. Which of the two uses a closure? How can you tell?
    
    Counter1 because the function is nested within another function.

  3. In what scenario would the counter1 code be preferable? In what scenario would 
     counter2 be better?  

     counter1 could start a counter if it was needed inside function scope and counter2 was if you wanted counter counter outside of function scope. 
...
function finalScore(inning, number){
  ...
  for(let  i = 0; i < number ; i++) {
    ...
    final.Home = inning() + final.Home
    final.Away = inning() + final.Away 
...
function scoreboard(getInningScore, inningCB, number) {
  ...
  for(let i = 0; i < number; i++){
    homeScore = homeScore + getInningScore(inningCB).Home
    awayScore = awayScore + getInningScore(inningCB).Away
